...
var user=""
			...
			window.onload = function () {
				...
				log=JSON.parse(localStorage.getItem(id.innerHTML+user));
				if (log==null)
					log=[]
...
//				console.log(history_arr)
				fetch('/chat', {
					...
					body: JSON.stringify({current_user:user,option:opt,system:system,question:tmp_question,history:JSON.stringify(history_arr)}),
				}).then(response => {
			...
			function clearChat(){
				localStorage.removeItem(id.innerHTML+user)
				log=[]
...
//                  alert(counter.toString());
                    ...
                    if(isbool == true){
				        ...
				        log[counter]={"role": "user", "content": ""}
//				        log.splice(counter,1)
				        localStorage.setItem(id.innerHTML+user,JSON.stringify(log))
